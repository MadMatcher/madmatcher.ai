...
export const GET: APIRoute = async () => {
  ...
  lines.push(
    "- Runtime: Apache Spark, or pandas for small data; batch pipelines plus real-time single-record serving (MadMatcher-Pro); scales to 100M+ tuples per table.",
  );
  lines.push(
  ...
  lines.push(
    `- Commercial: ${PRO.name}, premium features for production jobs (crash recovery, progress tracking, semantic blocking and matching), plus a real-time serving add-on, available now; ${CONSULTING.name}, available now.`,
  );
  lines.push("");
  ...
  for (const p of PRODUCTS) {
    lines.push(
      `- [${p.name}](${abs("/products#" + p.slug)}): ${p.role}. ${p.summary}`,
    );
  }
  lines.push("");
  ...
  lines.push(
    `- [Products](${abs("/products")}): the three tools that make up MadMatcher.`,
  );
  lines.push(
    `- [How it works](${abs("/how-it-works")}): the pipeline steps explained, from blocking through labeling to matching.`,
  );
  lines.push(
    `- [Why MadMatcher](${abs("/why-madmatcher")}): how the approach differs, and where it does not fit.`,
  );
  lines.push(
    `- [Compare approaches](${abs("/compare")}): entity matching approaches compared by principle.`,
  );
  lines.push(
    `- [What is entity matching](${abs("/about/entity-matching")}): a practical guide.`,
  );
  lines.push(
    `- [About](${abs("/about")}) · [Team](${abs("/about/team")}) · [Contact](${abs("/contact")})`,
  );
  lines.push(
    `- [LLM info](${abs("/llm-info")}): canonical, authoritative facts for AI assistants.`,
  );
  lines.push("");
  ...
  for (const g of glossary.sort((a, b) =>
    ...
    lines.push(
      `- [${g.data.term}](${abs("/glossary/" + g.id)}): ${g.data.definition}`,
    );
  }
  lines.push("");
  ...
  for (const u of useCases.sort((a, b) => a.data.order - b.data.order)) {
    lines.push(
      `- [${u.data.title}](${abs("/use-cases/" + u.id)}): ${u.data.summary}`,
    );
  }
  lines.push("");
  ...
  for (const d of docs.sort((a, b) => a.data.order - b.data.order)) {
    lines.push(
      `- [${d.data.title}](${abs("/docs/" + d.id)}): ${d.data.description}`,
    );
  }
  lines.push("");
  ...
  for (const b of blogSorted) {
    lines.push(
      `- [${b.data.title}](${abs("/blog/" + b.id)}): ${b.data.description}`,
    );
  }
  lines.push("");
